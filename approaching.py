# -*- coding: utf-8 -*-
"""
Created on Thu Jan  5 20:26:28 2017
...
#导入数据
data=xlrd.open_workbook('noise.xls')#50度进场
table1=data.sheets()[4]
table2=data.sheets()[3]
nrows=table1.nrows#行数
ncols=table1.ncols#列数
time=table1.row_values(0)[1:]
PNLT_tem=table1.row_values(25)[1:]
...
#作图
plt.plot(time,PNLT)
plt.plot(time,PNLT_tem)
plt.xlabel('时间(秒)',fontproperties=myfont)
plt.ylabel('有效感觉噪声级(分贝)',fontproperties=myfont)
plt.title('进近噪声',fontproperties=myfont)
plot1=plt.plot(time[3],PNLT_tem[3],'go',label='in point')
plot2=plt.plot(time[13],PNLT_tem[13],'ro',label='out point')
plt.show()